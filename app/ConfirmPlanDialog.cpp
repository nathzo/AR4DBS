    ...
    : QDialog(parent), m_scanMode(mode == Mode::Scan)
{
    ...
    setStyleSheet(
        ...
        "QDoubleSpinBox {"
        "  background: #2a2b2d; color: #e0e0e0;"
        "  border: 1px solid #444; border-radius: 6px; padding: 0px 12px;"
...
{
    logMemoryState("ConfirmPlanDialog::destructor");
    DebugLogger::logEvent("ConfirmPlanDialog", "destructor: entered, thread=" + QString::number((quint64)QThread::currentThreadId()));

    int childCount = findChildren<QObject*>().count();
    DebugLogger::logEvent("ConfirmPlanDialog", "destructor: found " + QString::number(childCount) + " child objects");
    ...
    DebugLogger::logEvent("ConfirmPlanDialog", "destructor: cleared accessibility on " + QString::number(widgetCount) + " widgets");
